...
# mlops.dev/jupyter-notebook-validator-operator-bundle:$VERSION and mlops.dev/jupyter-notebook-validator-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/takinosh/jupyter-notebook-validator-operator
...
# FBC catalog directory and image settings.
CATALOG_DIR ?= catalog
...
# Build a catalog image from the FBC directory.
# Uses catalog/catalog.Dockerfile which is committed to the repo.
.PHONY: catalog-build
catalog-build: opm ## Build catalog image from FBC directory.
	docker build -f $(CATALOG_DIR)/catalog.Dockerfile -t $(CATALOG_IMG) .
